...
import { branchDisplay, resolveGitRepositoryContext, worktreeLabel } from "./codex-context.mjs";
...
export async function documentMeta(filePath, options = {}) {
  ...
  const [branch, head, gitContext] = repoRoot
    ? await Promise.all([
      git(["branch", "--show-current"], repoRoot),
      git(["rev-parse", "--short", "HEAD"], repoRoot),
      resolveGitRepositoryContext(repoRoot),
    ])
    : ["", "", null];
