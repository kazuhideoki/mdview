...
- GFM、KaTeX によるインライン `$...$`／ブロック `$$...$$` 数式、Shiki のコードハイライト、Mermaid、D2、相対画像
- root Codex の `UserPromptSubmit` で基準状態を保存し、同じターンの正常な `Stop` までに変更された Markdown だけ再描画
...
Reader はワークツリーを閲覧の単位にします。左サイドバーでワークツリーを固定してから、その時点に存在するMarkdownを選びます。`P` でワークツリー全体の古い作業時点へ戻り、`N` で新しい作業時点へ進みます。通常の作業時点は、root Codex がユーザーリクエストを受けてから正常な `Stop` までにMarkdownを変更したターンです。変更のないターン、サブエージェントの終了、マージ参照を持たないGit同期点はN/Pに加えません。マージ元の作業履歴へ入り、マージ先へ戻るために必要な同期点は残します。除外対象の古いURLを直接開いた場合と、表示対象の作業が一つもない既存ワークツリーでは、保存済みスナップショットを読めるよう、その現在点だけを互換表示します。選択中のファイルが移動先にも存在する場合は、そのファイルを保ったまま内容を切り替えます。各作業時点の `Changes` は、そのターンの編集開始時点との差分です。
...
root Codex の `UserPromptSubmit` から、同じ `session_id + turn_id` の最初の正常な `Stop` までの内容差分を検出します。サブエージェントのhook、Markdown差分がないStop、完了済みturnへ後から届いたStopは履歴化しません。この間にCodex以外のプロセスが行った編集は区別できませんが、プロンプト送信前に存在した未記録変更はそのターンのChangesへ含めません。同じworktreeで複数のroot turnを並行実行した場合も編集の帰属は分離できないため、並行作業には別worktreeが必要です。また、interrupt / errorで `Stop` が発火しなかったターンは自動表示されません。既存履歴にはroot／subagentの区別が保存されていないため、古いhook履歴の表示判定は保存済みのsource・changes・merge参照によるbest-effortです。古い基準状態は7日で削除します。
