...
Treat one root Codex `UserPromptSubmit` through its matching successful `Stop` as one reader work-history unit, and add it only when Markdown changed during that interval. Exclude subagent hook events, no-change turns, and ordinary repository-sync revisions from N/P; retain merge-boundary sync nodes only when they are required to enter merged worktree history and return to the destination.
